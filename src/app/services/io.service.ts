...
import { BitfrostDatabase, DataLoaderObject } from '../interface/database';
...
export class IoService {
  ...
  bitfrostDatabase: BitfrostDatabase;
  storage = Storage;
  ...
  async initDb(): Promise<boolean> {
    try {
      this.bitfrostDatabase = this.plt.isCordova ? this.databaseService : this.localDatabaseService;

      if (this.plt.isCordova) {
        await this.bitfrostDatabase.create();
      }
  ...
  loadData(): Promise<DataLoaderObject> {
    if (this.bitfrostDatabase) return this.bitfrostDatabase.fetchData();
  ...
  async updateAccount(acc: Acc, secret: string, alog = false): Promise<Acc> {
    try {
      ...
      await this.bitfrostDatabase.updateAccount(encrypted);
  ...
  async updateMasterSeed(msed: MasterSeed, secret: string): Promise<MasterSeed> {
    try {
      ...
      await this.bitfrostDatabase.updateMasterSeed(encrypted);
      return msed;
    } catch (err) {
      throw err;
    }
  ...
  async addAccount(account: Acc): Promise<Acc> {
    ...
    try {
      ...
      const dbAccount = this.bitfrostDatabase.account;

      const a = !!dbAccount
        ? await this.bitfrostDatabase.updateAccount(compareValuesWith<Acc>(dbAccount, account))
        : await this.bitfrostDatabase.addAccount(encrypted);
  ...
  addAccountLog(alog: AccLog): Promise<void> {
    return this.bitfrostDatabase.addAccountLog(alog);
  }

  getAccountLog(uid: UserID): Promise<AccLog[]> {
    return this.bitfrostDatabase.getAccountLog(uid);
  }

  getLatestAccountLog(uid: UserID): Promise<AccLog> {
    return this.bitfrostDatabase.getLatestAccountLog(uid);
  }

  addWallet(wallet: Wallet): Promise<Wallet> {
    return this.bitfrostDatabase.addWallet(wallet).then(() => wallet);
  }

  addAbi(abi: Abi): Promise<Abi> {
    return this.bitfrostDatabase.addAbi(abi).then(() => abi);
  }
  ...
  getAbi(contractAddress: string, type: WalletType): string {
    ...
    const abi = this.bitfrostDatabase.abis.find(
  ...
  async addMasterSeed(msed: MasterSeed, secret: string): Promise<MasterSeed> {
    try {
      ...
      await this.bitfrostDatabase.addMasterSeed(encrypted);
      return msed;
    } catch (err) {
      throw err;
    }
  ...
  addWalletAddress(address: WalletAddress): Promise<Wallet> {
    return this.bitfrostDatabase.addWalletAddress(address).then(wa => {
  ...
  addTutorials(tutorials: Tutorials): Promise<Tutorials> {
    return this.bitfrostDatabase.addTutorials(tutorials);
  }

  updateWallet(wallet: Wallet): Promise<Wallet> {
    return this.bitfrostDatabase.updateWallet(wallet);
  }
  ...
  updateTutorials(tutorials: Tutorials): Promise<Tutorials> {
    return this.bitfrostDatabase.updateTutorials(tutorials);
  }

  alertTable(): Promise<void> {
    return this.bitfrostDatabase.alertTable();
  }

  // @todo finish this change
  updateAddressTx(name, address, txid) {
    const { uid } = this.authProvider.accountValue;
    // this.bitfrostDatabase
    //   .updateTransaction(uid, name, address, txid)
    //   .then(_ => this.getWallets(uid));
  }

  // updateCacheTransaction(uuid: UUID, tx: Transaction): Promise<void> {
  //   return this.bitfrostDatabase.updateTransactionCache(uuid, tx);
  // }

  removeWallet(wallet: Wallet): Promise<Wallet[]> {
    return this.bitfrostDatabase.removeWallet(wallet);
  }

  removeWalletsOf(uid: UserID): Promise<void> {
    return this.bitfrostDatabase.removeWalletsOf(uid);
  }

  removeAccount(): Promise<void> {
    return this.bitfrostDatabase.removeAccount();
  }

  removeTutorials(uid: UserID): Promise<void> {
    return this.bitfrostDatabase.removeTutorials(uid);
  }

  updateSettings(id: UserID, settings: Settings): Promise<Settings> {
    return this.bitfrostDatabase
  ...
  addSettings(settings: Settings): Promise<Settings> {
    return this.bitfrostDatabase.addSettings(settings);
  }

  readSettings(uid: UserID): Settings {
    try {
      return this.bitfrostDatabase.settings.find(s => s.uid === uid);
    } catch (err) {
      console.error(err);
  ...
  getWallets(uid?: UserID) {
    ...
    if (id.length > 0) {
      const wallets = this.bitfrostDatabase.wallets.filter(w => w.uid === id);
      this.walletsProvider.pushWallets(wallets);
      return wallets;
    } else {
      this.walletsProvider.pushWallets(this.bitfrostDatabase.wallets);
      return this.bitfrostDatabase.wallets;
    }
  }
  ...
  matchWallets(keyVal: Partial<Wallet>): Wallet[] {
    const isWallet = (w: Wallet) =>
      Object.entries(keyVal)
        ...
        .every(v => !!v);
    return this.bitfrostDatabase.wallets.filter(isWallet);
  }

  getWallet(uuid: UUID): Wallet {
    return this.bitfrostDatabase.wallets.find(w => w._uuid === uuid);
  }

  getWalletByCoinType(ticker: string, type: WalletType, uid: UserID): Wallet {
    return this.bitfrostDatabase.wallets.find(
  ...
  async getMasterSeed(id: UserID, secret: string): Promise<MasterSeed> {
    const msed = await this.bitfrostDatabase.getMasterSeed(id);
    if (!msed) return null;
  ...
  getTutorials(uid: UserID): Promise<Tutorials> {
    return this.bitfrostDatabase.getTutorials(uid);
  }
  ...
  async addTransactionOf(wallet: Wallet, updateTransaction: boolean): Promise<Transaction[]> {
    ...
    await Promise.all(newTxs.map(tx => this.bitfrostDatabase.addTransaction(tx)));
    ...
    await this.bitfrostDatabase.updateTransactions(updatedTxs);
  }

  removeTransactions(uuid: UUID): Promise<void> {
    return this.bitfrostDatabase.removeTransactions(uuid);
  }

  getTrasactionPageOf(wallet: Wallet, last = 1, limit = 15): Promise<Transaction[]> {
    return this.bitfrostDatabase.getWalletTransactionsPage(wallet._uuid, last, limit);
  }

  getTransactionsOf(wallet: Wallet): Promise<Transaction[]> {
    return this.bitfrostDatabase.getWalletTransactions(wallet._uuid);
  }
