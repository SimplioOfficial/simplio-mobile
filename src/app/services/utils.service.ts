...
import { InterestRate, Pool } from '@simplio/backend/interface/stake';
...
export class UtilsService {
  ...
  static isToken(type) {
    return isSolanaToken(type) || isErcToken(type) || isCustomToken(type);
  }
...
export const isCustomToken = (type: WalletType): boolean => {
  return type === WalletType.CUSTOM_TOKEN;
};

export const isToken = (type: WalletType): boolean => {
  return isSolanaToken(type) || isErcToken(type) || isCustomToken(type);
};
...
export const tokenId = (type: WalletType): string => {
    switch (type) {
...
export const getAPY = (pool: Pool): number => {
    const curr = Math.floor(Date.now() / 1000);
    const rates = pool.rate.filter(e => e.unixTimestamp <= curr);
    const rate = rates.reduce(function (prev, current) {
